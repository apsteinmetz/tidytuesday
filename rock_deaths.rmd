...
mortality_summary %>% group_by(year,General_Cause) %>%
  summarise(proportion=mean(proportion)) %>% 
  ggplot(aes(x=year,y=proportion,color=General_Cause))+geom_line()
```
...
us_deaths<-mortality_summary %>% 
  ...
  group_by(General_Cause) %>% 
  summarise(US=sum(proportion)/100)
...
comp_deaths<-cause_table %>% 
  ...
  na.omit() %>% 
  mutate(Rockers=Rockers/sum(Rockers),US=proportion/100) %>% 
  {.}
